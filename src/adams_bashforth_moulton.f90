...
module abm_integrator
  ...
  !===================================================================!
  ! A derived type for the abm coefficients
  !===================================================================!
  ...
  ! Interface for the constructor of abm_coeff type
  interface abm_coeff
     module procedure construct_abm_coeff
  end interface abm_coeff
...
contains
  ...
  !===================================================================!
  ! Constructor for ABM coeff object
  !===================================================================!
  
  type(abm_coeff) function construct_abm_coeff( max_order ) result( this )
    ...
    if (this % max_order .eq. 3) then
       ...
       print *,  "Wrong max_abm_order:", this % max_order
       stop
    end if

  end function construct_abm_coeff
  
  !===================================================================!
  ! Destructor for ABM coeff object
  !===================================================================!
  
  subroutine destruct( this ) 
    
    class(abm_coeff) :: this
  ...
  pure integer function getOrder(this, k, d )
    ...
    class(abm_coeff), intent(in)    :: this
...
!!$    dfdx = dfdx + matmul(this % psi(2,:), dRdX)
  ...
  type(abm) function initialize( system, tinit, tfinal, h, second_order, max_abm_order )  result (this)
    ...
    !-----------------------------------------------------------------!
    ! Create the ABM-coeff object
    !-----------------------------------------------------------------!
    
    this % coeff = abm_coeff(this % max_abm_order)
