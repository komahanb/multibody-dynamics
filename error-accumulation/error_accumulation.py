...
def parse_error(fil):
    ...
    for line in data:
        cols = line.split()
        abs_err.append(float(cols[1]))
        rel_err.append(float(cols[2]))
...
time_steps = np.array([1,10,100,1000,10000,100000,1000000])

num_steps = 7
max_order = 4
num_methods = 4

error = np.zeros((num_methods,max_order,2,num_steps))

######################################################################
# Adams Bashforth Moulton
######################################################################

fil = "bdf1_error_accumulation.dat"
error[0,0,:,:] = parse_error(fil)
fil = "bdf2_error_accumulation.dat"
error[0,1,:,:] = parse_error(fil)
fil = "bdf3_error_accumulation.dat"
error[0,2,:,:] = parse_error(fil)
make_plot_bdf(time_steps, error[0,:,:,:], "bdf-error-accumulation.pdf")

fil = "abm1_error_accumulation.dat"
error[1,0,:,:] = parse_error(fil)
fil = "abm2_error_accumulation.dat"
error[1,1,:,:] = parse_error(fil)
fil = "abm3_error_accumulation.dat"
error[1,2,:,:] = parse_error(fil)
make_plot_abm(time_steps, error[1,:,:,:], "abm-error-accumulation.pdf")

fil = "dirk2_error_accumulation.dat"
error[2,0,:,:] = parse_error(fil)
fil = "dirk3_error_accumulation.dat"
error[2,1,:,:] = parse_error(fil)
fil = "dirk4_error_accumulation.dat"
error[2,2,:,:] = parse_error(fil)
make_plot_dirk(time_steps, error[2,:,:,:], "dirk-error-accumulation.pdf")

fil = "nbg2a_error_accumulation.dat"
error[3,0,:,:] = parse_error(fil)
fil = "nbg2b_error_accumulation.dat"
error[3,1,:,:] = parse_error(fil)
fil = "nbg2c_error_accumulation.dat"
error[3,2,:,:] = parse_error(fil)
fil = "nbg3_error_accumulation.dat"
error[3,3,:,:] = parse_error(fil)
make_plot_nbg(time_steps, error[3,:,:,:], "nbg-error-accumulation.pdf")

# plot all second order methods in one plot

#plotall_order1(time_steps, error[:,0,:,:], "order1-error-accumulation.pdf")
plotall_order2(time_steps, error[:,1,:,:], "order2-error-accumulation.pdf")
plotall_order3(time_steps, error[:,2,:,:], "order3-error-accumulation.pdf")
# plotall_order4(time_steps, error[:,3,:,:], "order2-error-accumulation.pdf")

#make_plot(func_idx, error[0,1,:,:], "abm1-grad-verify.pdf")
#make_plot(func_idx, error[0,2,:,:], "abm1-grad-verify.pdf")
